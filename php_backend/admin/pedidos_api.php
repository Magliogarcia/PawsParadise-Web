...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $accion === 'crear') {
        ...
        $stmt = $conn->prepare("INSERT INTO pedidos (usuario_id, total, metodo_pago, estado) VALUES (?, ?, ?, 'Pendiente')");
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && $accion === 'listar_admin') {
        ...
        $stmt = $conn->query("SELECT p.*, u.nombre, u.apellido, u.cedula FROM pedidos p JOIN usuarios u ON p.usuario_id = u.id ORDER BY p.id DESC");
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && $accion === 'listar_cliente') {
        ...
        $stmt = $conn->prepare("SELECT * FROM pedidos WHERE usuario_id = ? ORDER BY id DESC");
        $stmt->execute([$usuario_id]);
        $pedidos = $stmt->fetchAll(PDO::FETCH_ASSOC);

        // Buscar qué productos tiene cada pedido para mostrarlos
        foreach ($pedidos as &$pedido) {
            $stmtDet = $conn->prepare("SELECT pd.cantidad, pd.precio_unitario, pr.nombre, pr.imagen FROM pedido_detalles pd JOIN productos pr ON pd.producto_id = pr.id WHERE pd.pedido_id = ?");
